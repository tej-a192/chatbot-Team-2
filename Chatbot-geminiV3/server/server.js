...
if (!process.env.JWT_SECRET) {
  ...
  console.error(
    "!!! Please set it in your .env file before running:        !!!"
  );
  console.error(
  ...
  try {
    ...
    if (response.status === 200 && response.data?.status === "ok") {
      ...
      const embeddingModel =
        response.data.embedding_models?.document_embedding_model || "N/A";
      ...
      console.warn(
        `! Python RAG service responded but status is not OK: ${
          response.status
    ...
    if (error.code === "ECONNREFUSED") {
      console.warn(
        `  Connection refused at ${url}. Ensure the Python RAG service (e.g., server/rag_service/app.py) is running.`
      );
    } else if (
      ...
      console.warn(
        `  Connection timed out to ${url}. The Python RAG service might be slow to start or unresponsive.`
      );
    } else {
      console.warn(
        `  Error connecting to Python RAG Service: ${error.message}`
      );
    }
    console.warn(
      "  RAG-dependent features (document upload processing, context retrieval) will be unavailable."
    );
    return false;
  }
}
...
async function configureAndStart() {
  ...
  if (!mongoUri) {
    const answer = await askQuestion(
      `Enter MongoDB URI or press Enter for default (${DEFAULT_MONGO_URI}): `
    );
  ...
  if (!pythonRagUrl) {
    const answer = await askQuestion(
      `Enter Python RAG Service URL or press Enter for default (${DEFAULT_PYTHON_RAG_URL}): `
    );
