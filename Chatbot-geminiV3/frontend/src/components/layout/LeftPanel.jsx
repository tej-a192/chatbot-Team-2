...
import DocumentUpload from '../documents/DocumentUpload.jsx'; // For user's own documents
import DocumentList from '../documents/DocumentList.jsx';     // For user's own documents
import {
    PanelLeftClose, ChevronDown, ChevronUp, FilePlus, Settings2,
    Bot, BookOpen, Lightbulb, Library, Loader2 // Added Library for subject icon, Loader2
} from 'lucide-react';
import IconButton from '../core/IconButton.jsx';
import { motion, AnimatePresence } from 'framer-motion';
import toast from 'react-hot-toast';
import api from '../../services/api.js'; // Main API service for regular users

const PROMPT_PRESETS = [ /* ... your presets ... */
...
function LeftPanel() {
    const {
        ...
        selectDocumentForAnalysis, selectedDocumentForAnalysis, // For user's own docs for analysis tools
        selectedSubject, setSelectedSubject // <<< For selecting an admin doc as a subject
    } = useAppState();

    const [isPromptSectionOpen, setIsPromptSectionOpen] = useState(true);
    const [isSubjectSectionOpen, setIsSubjectSectionOpen] = useState(true); // For the new section
    const [isDocManagementOpen, setIsDocManagementOpen] = useState(true); // For user's own docs
    ...
    useEffect(() => { // For system prompt presets
    ...
    const fetchSubjects = useCallback(async () => {
        ...
        try {
            const response = await api.getSubjects(); // Fetches admin doc names
    ...
    useEffect(() => { // Fetch subjects on mount
        fetchSubjects();
    }, [fetchSubjects]);
    ...
    const handleUploadSuccessForUserDocs = () => { // Renamed for clarity
    ...
    const handleSubjectSelectionChange = (event) => {
        ...
        setSelectedSubject(newSubjectName === "none" ? null : newSubjectName);
        // AppStateContext's setSelectedSubject should handle clearing selectedDocumentForAnalysis
    };
            ...
            <div className="mb-4">
                ...
                <AnimatePresence>
                    {isSubjectSectionOpen && (
                        ...
                        >
                            {isLoadingSubjects ? (
                                <div className="flex items-center justify-center text-xs text-text-muted-light dark:text-text-muted-dark py-2">
                                    <Loader2 size={16} className="animate-spin mr-2" /> Loading subjects...
                                </div>
                            ) : availableSubjects.length > 0 ? (
                                <>
                                    <label htmlFor="subject-select-dropdown" className="block text-xs font-medium text-text-muted-light dark:text-text-muted-dark mb-1">
                                        Focus chat on subject:
                                    </label>
                                    <select
                                        id="subject-select-dropdown"
                                        value={selectedSubject || "none"} // Handle null by defaulting to "none"
                                        onChange={handleSubjectSelectionChange}
                                        className="input-field text-xs py-1.5 w-full"
                                    >
                                        <option value="none">-- None (General Chat) --</option>
                                ...
                                <p className="text-xs text-center text-text-muted-light dark:text-text-muted-dark py-2">
                                    No subjects configured by admin yet.
                                </p>
                            )}
                        </motion.div>
                    )}
                    ...
                    {isDocManagementOpen && (
                        <motion.div /* ... */ key="doc-management-content" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2, ease: "easeInOut" }} className="flex-grow flex flex-col overflow-hidden p-3 bg-surface-light dark:bg-surface-dark border border-border-light dark:border-border-dark rounded-md shadow-inner">
                            ...
                            <div className="mt-3 flex-grow overflow-y-auto custom-scrollbar">
                                {/* You might want a label here if you re-add filtering */}
                                <DocumentList
                                    key={docListKey}
