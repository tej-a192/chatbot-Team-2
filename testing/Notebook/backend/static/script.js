...
// --- Listen for username from parent window ---
// window.addEventListener('message', (event) => {
//   if (event.origin !== "http://localhost:3000") return; // security check

//   const { type, username } = event.data;
//   if (type === 'SET_USERNAME' && username) {
//     console.log('Username received from parent:', username);
//     localStorage.setItem('username', username);

//     // If your app depends on username being available immediately
//     // you can call your loading function here:
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const kgCheckbox = document.getElementById('use-kg-checkbox');
    ...
    window.addEventListener('message', (event) => {
      ...
      const { type, username, sessionId } = event.data;
      if (type === 'SET_USER_AND_SESSION' && username && sessionId) {
        console.log('Session ID received from parent:', sessionId);
        console.log('Username received from parent:', username);
        localStorage.setItem('username', username);
        localStorage.setItem('aiTutorSessionId', sessionId);
        clearChatHistory();
        setSessionIdDisplay(sessionId);
        // loadAndPopulateDocuments(); //Donot load for every change session..Uncomment it, if it finds useful  
      }
    });
    ...
    function updateControlStates() {
        ...
        const canSelectAnalysis = isDbReady && (allFiles.uploaded.length > 0);
    ...
    function updateAnalysisDropdown() {
        ...
        const hasFiles = allFiles.uploaded.length > 0;
    ...
    async function loadAndPopulateDocuments() {
        ...
        try {
            ...
            console.log(`Loaded ${allFiles.uploaded.length} uploaded docs.`);
    ...
    async function handleAnalysis(analysisType) {
        ...
        console.log(`Retrieving analysis: Type=${analysisType}, File=${filename}`);
        ...
        try {
            ...
            const response = await fetch(`${API_BASE_URL}/handleAnalysis`, {
                method: 'POST',
                // headers: { 'Content-Type': 'application/json' },
                // body: JSON.stringify({ filename, analysis_type: analysisType, username }),
                body: formData,
            });
    ...
    async function handleSendMessage() {
        ...
        if (!query) return;
        isKgEnabled = kgCheckbox.checked; // Check KG state
        // console.log("Sending message:", query, "KG Enabled:", isKgEnabled);
        ...
        try {
            const response = await fetch(`${API_BASE_URL}/chat`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ query: query, session_id: sessionId, isKgEnabled: isKgEnabled, username: localStorage.getItem('username') }),
            });
