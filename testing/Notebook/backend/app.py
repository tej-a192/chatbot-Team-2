...
def get_documents():
    ...
    """
    Returns a sorted list of uploaded PDF filenames for the given user.
    Expects 'username' in form data (POST).
    """

    if not app_db_ready:
        logger.error("Documents request failed: Database not initialized.")
        return jsonify({"uploaded_files": [], "errors": ["Database connection failed."]}), 503

    username = request.form.get('username', '').strip()
    if not username:
        logger.warning("Documents request missing 'username' parameter.")
        return jsonify({"uploaded_files": [], "errors": ["Missing 'username' parameter."]}), 400

    try:
        # Query the user's document list from the database
        conn = database.get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT documents FROM users WHERE username = ?", (username,))
        row = cursor.fetchone()
        conn.close()

        if row and row["documents"]:
            try:
                uploaded_files = sorted(json.loads(row["documents"]))
            except Exception as e:
                logger.error(f"Error parsing documents JSON for user '{username}': {e}")
                uploaded_files = []
        else:
            logger.info(f"No documents found for user '{username}'. Returning empty list.")
            uploaded_files = []
        ...
        logger.info(f"Returning {len(uploaded_files)} uploaded files for user '{username}'.")
        return jsonify(response_data)

    except Exception as e:
        logger.error(f"Error retrieving documents for user '{username}': {e}", exc_info=True)
        return jsonify({"uploaded_files": [], "errors": ["Database error retrieving documents."]}), 500


def generate_all_analyses_async(filename, username, text):
    """
    Generates FAQ, Topics, and Mindmap analyses for a document in the background,
    and saves them to the document_analysis table.
    """
    logger.info(f"[Analysis] Starting background analysis for '{filename}' (user: {username})")
    analysis_types = ['faq', 'topics', 'mindmap']
    results = {}
    for analysis_type in analysis_types:
        try:
            # Use ai_core to generate analysis (reuse your previous logic)
            content, thinking = ai_core.generate_document_analysis(filename, analysis_type, username, text)
            results[analysis_type] = content
            logger.info(f"[Analysis] Generated {analysis_type} for '{filename}' (user: {username})")
        except Exception as e:
            logger.error(f"[Analysis] Error generating {analysis_type} for '{filename}' (user: {username}): {e}")
            results[analysis_type] = None

    # Save all analyses to the database in one call
    try:
        ...
        logger.info(f"[Analysis] Saved all analyses for '{filename}' (user: {username}) to database.")
    except Exception as e:
        logger.error(f"[Analysis] Error saving analyses to DB for '{filename}' (user: {username}): {e}")


def send_text_to_kg_async(filename, text, username):
    def _send():
        try:
            response = requests.post(
                "http://localhost:8001/start-kg",
                json={
                    "filename": filename,
                    "username": username,
                    "text": text
        ...
        #         logger.info(f"[KG] Triggered KG generation for {filename}")
        #     else:
        #         logger.error(f"[KG] Failed to trigger KG. Status: {response.status_code}, Error: {response.text}")
        # except Exception as e:
        #     logger.error(f"[KG] Exception while sending to KG: {e}")
            if response.status_code == 200:
                kg_data = response.json()
                kg_json = kg_data.get("kg_json")
                user = kg_data.get("username")
                fname = kg_data.get("filename")
                # --- Save KG JSON to Notebook/backend/KG_OUTPUTS/username/filename.json ---
                output_dir = os.path.join(os.path.dirname(__file__), "KG_OUTPUTS", user)
                os.makedirs(output_dir, exist_ok=True)
                output_path = os.path.join(output_dir, f"{fname}.json")
                with open(output_path, "w", encoding="utf-8") as f:
                    json.dump(kg_json, f, indent=2, ensure_ascii=False)
                logger.info(f"[KG] KG JSON saved to {output_path}")
            else:
                logger.error(f"[KG] Failed to trigger KG. Status: {response.status_code}, Error: {response.text}")
        except Exception as e:
            logger.error(f"[KG] Exception while sending to KG: {e}")
...
def upload_file():
    ...
    logger.debug(f" ************** File upload received: {file.filename}")
    ...
    # --- Extract Text from PDF (in-memory, do not save to disk) ---
    try:
        # file.stream is a file-like object, pass directly to extractor if supported
        logger.info(f"Processing uploaded file in-memory: {filename}...")
        text = ai_core.extract_text_from_pdf(file.stream)
        if not text:
        ...
        # Analysis generation
        if username:
            # Start background thread for analysis generation
            threading.Thread(
                target=generate_all_analyses_async,
                args=(filename, username, text),
                daemon=True
            ).start()
            logger.info(f"[Analysis] Background analysis thread started for '{filename}' (user: {username})")
        else:
            logger.error(f"Client Error, User is not defined")
            return jsonify({"error": f"File '{filename}' processed, but failed to Generate Analysis."}), 500


        # Trigger KG microservice asynchronously
        if username:
            send_text_to_kg_async(filename, text, username)
        else:
            logger.error(f"Client Error, User is not defined")
            return jsonify({"error": f"File '{filename}' processed, but failed to Generate Knowledge graph."}), 500
        ...
        if not documents:
            logger.error(f"Could not create document chunks for {filename}, although text was extracted. Cannot add to knowledge base for chat.")
        ...
        # --- Append filename to user's document list in SQLite ---
        if username:
            try:
                database.add_document_to_user(username, filename)
                logger.info(f"Appended '{filename}' to user '{username}' document list.")
            except Exception as e:
                logger.error(f"Failed to append document to user '{username}': {e}")
...
#     logger.info(f"Analysis request received: type='{analysis_type}', file='{filename}' for user '{username}'")
...
#         analysis_content, thinking_content = ai_core.generate_document_analysis(filename, analysis_type, username)
...
@app.route('/handleAnalysis', methods=['POST'])
def handle_analysis():
    """
    Retrieves the requested analysis (FAQ, Topics, Mindmap) for a user and document.
    Expects query parameters: username, filename, analysis_type.
    """
    username = request.form.get('username', '').strip()
    filename = request.form.get('filename', '').strip()
    analysis_type = request.form.get('analysis_type', '').strip().lower()

    allowed_types = ['faq', 'topics', 'mindmap']
    if not username or not filename or analysis_type not in allowed_types:
        logger.warning(f"[handleAnalysis] Invalid parameters: username={username}, filename={filename}, analysis_type={analysis_type}")
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT faq, topics, mindmap FROM document_analysis WHERE filename = ? AND username = ?",
            (filename, username)
        )
        row = cursor.fetchone()
        conn.close()
        ...
        if not row:
            logger.info(f"[handleAnalysis] No analysis found for '{filename}' (user: {username})")
            return jsonify({"error": "Analysis not found. It may still be generating. Please try again later."}), 404

        analysis_content = row[analysis_type]
        if not analysis_content:
            logger.info(f"[handleAnalysis] {analysis_type} not ready for '{filename}' (user: {username})")
            return jsonify({"error": f"{analysis_type.capitalize()} is still being generated. Please try again later."}), 202

        logger.info(f"[handleAnalysis] Returning {analysis_type} for '{filename}' (user: {username})")
        return jsonify({
            "content": analysis_content,
        ...
        logger.error(f"[handleAnalysis] Error retrieving analysis for '{filename}' (user: {username}): {e}", exc_info=True)
        return jsonify({"error": "Server error retrieving analysis."}), 500
...
# def chat():
    ...
    isKgEnabled = data.get('isKgEnabled') # Get KG enabled status from request
    ...
    try:
        ...
        logger.warning(f"Received invalid session_id format: '{session_id}'.")
        return jsonify({"error": "Invalid session ID format."}), 400

    # Log entry with session info
    # logger.info(f"Processing chat query (Session: {session_id}, New: {is_new_session}): '{query[:150]}...'")
    logger.info(f"Processing chat query (Session: {session_id}): '{query[:150]}...'")
    # --- Log User Message ---
    ...
    try:
        ...
        kg_context = None
        if isKgEnabled:
            kg_context = ai_core.get_kg_context_for_query(query)
            if kg_context:
                logger.info(f"KG context retrieved for query '{query[:50]}...'")
                # --- Add KG context to context_text ---
                context_text = f"[Knowledge Graph Context]\n{kg_context}\n\n{context_text}"
            else:
                logger.info(f"No KG context found for query '{query[:50]}...'")
        else:
            logger.debug(f"KG context retrieval skipped for query '{query[:50]}'... (isKgEnabled: {isKgEnabled}).")
...
# In app.py, find and replace the existing chat() function
...
def chat():
    ...
    if not app_vector_store_ready and config.RAG_CHUNK_K > 0: # Only warn if RAG is expected/configured
        # This warning is fine, chat can still proceed without RAG
        logger.warning("Chat request proceeding, but vector store is not loaded/ready. RAG context will be empty or unavailable.")
    ...
    isKgEnabled = data.get('isKgEnabled') # Boolean from frontend checkbox
    username = data.get('username') # <-- Ensure frontend sends this with the query
    ...
    # --- Validate Username (Crucial for user-specific data like KG/docs) ---
    if not username or not username.strip():
         logger.error("Chat request missing 'username' parameter.")
         return jsonify({"error": "Username is required for chat."}), 400
    username = username.strip() # Use stripped username

    # --- Session Management ---
    # (Your existing session ID handling logic - ensure it's robust)
    if not session_id:
        logger.warning(f"Chat request received without session_id for user {username}. This should not happen if session is managed.")
        # Decide if this should be an error or generate new session ID
        # For now, let's require it based on your structure
        return jsonify({"error": "Session ID is required."}), 400
    try:
        uuid.UUID(session_id, version=4) # Validate format
    except (ValueError, TypeError, AttributeError):
        logger.warning(f"Received invalid session_id format: '{session_id}' for user {username}.")
        return jsonify({"error": "Invalid session ID format."}), 400


    # Log entry with session info
    logger.info(f"Processing chat query (Session: {session_id}, User: {username}, KG Enabled: {isKgEnabled}): '{query[:150]}...'")

    # --- Log User Message ---
    user_message_id = None # Initialize to None
    try:
        user_message_id = database.save_message(session_id, 'user', query, None, None)
        if not user_message_id:
             logger.error(f"Failed to save user message to database for session {session_id}, user {username}.")
    except Exception as db_err:
         logger.error(f"Database error occurred while saving user message for session {session_id}, user {username}: {db_err}", exc_info=True)
         # Decide if critical - currently logs and continues.
    ...
    try:
        ...
        if app_vector_store_ready and config.RAG_CHUNK_K > 0:
            logger.debug(f"Performing RAG search (session: {session_id}, user: {username})...")
            # ai_core.perform_rag_search returns: context_docs, formatted_context_text, context_docs_map
            context_docs, context_text, context_docs_map = ai_core.perform_rag_search(query)
            if context_docs:
                 logger.info(f"RAG search completed. Found {len(context_docs)} unique context chunks for session {session_id}.")
             ...
             logger.warning(f"Skipping RAG search for session {session_id}, user {username}: Vector store not ready.")
             context_text = "Knowledge base access is currently unavailable; providing general answer."
        else: # RAG_CHUNK_K <= 0
             logger.debug(f"Skipping RAG search for session {session_id}, user {username}: RAG is disabled.")
             context_text = "Document search is disabled; providing general answer."


        # --- KG Retrieval (if enabled) ---
        kg_context = None
        if isKgEnabled:
            logger.info(f"KG enabled. Attempting KG context retrieval for user {username}...")

            # === DETERMINE ACTIVE FILENAME ===
            # This is a critical point: how do we know which document the user is chatting about?
            # Option 1 (Simple Proxy - used here for example): Get the last uploaded file for the user from the DB.
            # Option 2 (More Robust): Frontend sends the currently active document filename with the chat query.
            # Option 3 (Even More Robust): Store the active document filename with the user's session in the DB.
            ...
            try:
                ...
                if row and row["documents"]:
                    ...
                    if docs:
                        # Get the last document in the list as a proxy for the active one.
                        # This assumes a single relevant document per user session for KG chat.
                        active_filename = docs[-1]
                        logger.debug(f"Determined potential active filename from user documents: {active_filename} for user {username}")
                    else:
                        logger.info(f"User {username} has no uploaded documents listed. Cannot retrieve KG context.")

            except Exception as db_err:
                logger.error(f"Database error retrieving user documents for KG context for user {username}: {db_err}", exc_info=True)
                # Do NOT set active_filename, KG context will be skipped.

            if active_filename:
                # === Call the UPDATED get_kg_context_for_query with query, filename, and username ===
                kg_context = ai_core.get_kg_context_for_query(query, active_filename, username)
                
                # logger.info(f"[KG Context] Retrieved context: {kg_context}")

                if kg_context:
                    logger.info(f"KG context retrieved for query '{query[:50]}...' for file '{active_filename}'.")
                elif kg_context is None:
                     logger.info(f"No relevant KG context found or KG file missing/empty for file '{active_filename}'.")
                # If kg_context is an error string, it will be handled when combined.

            else:
                 logger.warning(f"KG enabled, but no active filename determined for user {username}. Skipping KG retrieval.")
                 # kg_context remains None

        else:
            logger.debug(f"KG is disabled for session {session_id}.")


        # --- Combine RAG and KG Contexts ---
        # Start with RAG context. Add KG context if available.
        # Format the combined context clearly for the LLM.
        combined_context = ""
        if kg_context:
            combined_context += "[Knowledge Graph Context]\n"
            combined_context += kg_context # This might be formatted data or an error message
            combined_context += "\n\n---\n\n" # Separator
            logger.debug("KG context included in combined context.")
            # logger.info(f"[KG Context] Combined context: {combined_context}")
            logger.debug(f"[KG Context] Combined context: {combined_context}")
        combined_context += "[Document Search Results]\n"
        combined_context += context_text # This is the formatted RAG chunks text
        combined_context += "\n\n---\n\n" # Separator


        # 2. Synthesize Response using LLM (pass combined_context)
        logger.debug(f"Synthesizing chat response (session: {session_id}, model: {config.OLLAMA_MODEL})...")
        # === Pass the COMBINED context to synthesis ===
        bot_answer, thinking_content = ai_core.synthesize_chat_response(query, combined_context)
        # Log if synthesis itself failed (returned error message)
        if bot_answer.startswith("Error:") or "encountered an error" in bot_answer:
             logger.error(f"LLM Synthesis failed for session {session_id}. Response: {bot_answer}")
        ...
        # 3. Extract References (only if RAG provided context and answer is not an error message)
        # Check if context_docs_map has items and bot_answer doesn't indicate a primary error
        if rag_context_retrieved and context_docs_map and not (bot_answer.startswith("Error:") or "[AI Response Processing Error:" in bot_answer or "encountered an error" in bot_answer.lower()):
            logger.debug(f"Extracting references from bot answer (session: {session_id})...")
            references = utils.extract_references(bot_answer, context_docs_map)
            if references:
                logger.info(f"Extracted {len(references)} unique references for session {session_id}.")
            # else: logger.debug("No citation markers found in the bot answer.")
             ...
             logger.debug(f"Skipping reference extraction for session {session_id}: No RAG context provided or bot answer indicates an error.")


        # --- Log Bot Response (including thinking and references) ---
        bot_message_id = None # Initialize to None
        try:
            # Save the final answer, parsed references (JSON), and thinking content
            bot_message_id = database.save_message(
                session_id, 'bot', bot_answer, references, thinking_content # Pass thinking here
            ...
            if not bot_message_id:
                 logger.error(f"Failed to save bot response to database for session {session_id}, user {username}.")
        except Exception as db_err:
             logger.error(f"Database error occurred while saving bot response for session {session_id}, user {username}: {db_err}", exc_info=True)
        ...
        logger.debug(f"Returning chat response payload for session {session_id}.")
        return jsonify(response_payload), 200 # OK

    except Exception as e:
        # Catch unexpected errors during the RAG/Synthesis pipeline
        logger.error(f"Unexpected error during chat processing pipeline for session {session_id}, user {username}: {e}", exc_info=True)
        # Construct a user-friendly error message
        error_message = f"Sorry, an unexpected server error occurred ({type(e).__name__}). Please try again or contact support if the issue persists."
        # Attempt to log this severe error to the chat history as well
        try:
            # Include error details in thinking for debugging via history
            error_thinking = f"Unexpected error in /chat: {type(e).__name__}: {str(e)}"
            database.save_message(session_id, 'bot', error_message, None, error_thinking)
        except Exception as db_log_err:
            logger.error(f"Failed even to save the error message to DB for session {session_id}: {db_log_err}")
