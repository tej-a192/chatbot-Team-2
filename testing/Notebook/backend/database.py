...
def init_db():
    ...
    try:
        ...
        # --- NEW: Create users table with only username and documents ---
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS users (
                username TEXT PRIMARY KEY NOT NULL,
                documents TEXT DEFAULT '[]' -- JSON array of uploaded document names
            )
        ''')
        logger.info("Table 'users' checked/created.")

        # --- ADD THIS: Create document_analysis table ---
        cursor.execute('''
        ...
        ''')
        logger.info("Table 'document_analysis' checked/created.")
        # --- END OF NEW TABLES ---
...
def add_document_to_user(username: str, filename: str) -> bool:
    """
    Appends a filename to the user's document list in the users table.
    Creates the user if not present.

    Args:
        username (str): The username.
        filename (str): The document filename to add.

    Returns:
        bool: True if successful, False otherwise.
    """
    if not username or not filename:
        logger.error("Username and filename must be provided to add_document_to_user.")
        return False
    ...
    try:
        ...
        if row:
            ...
            if filename not in docs:
                ...
                cursor.execute("UPDATE users SET documents = ? WHERE username = ?", (json.dumps(docs), username))
        else:
            # User does not exist, create with this document
            docs = [filename]
            cursor.execute("INSERT INTO users (username, documents) VALUES (?, ?)", (username, json.dumps(docs)))
        conn.commit()
        logger.info(f"Added document '{filename}' to user '{username}'.")
        return True
    except sqlite3.Error as e:
        logger.error(f"Database error in add_document_to_user for user '{username}': {e}", exc_info=True)
        if conn:
            conn.rollback()
        return False
    finally:
...
def save_analysis_to_db(filename: str, username: str, faq: str | None = None, topics: str | None = None, mindmap: str | None = None) -> bool:
    """
    Saves or updates the analysis data for a document and user.

    Args:
        filename (str): The document filename.
        username (str): The username.
        faq (str | None): FAQ data as a string (possibly JSON or text).
        topics (str | None): Topics data as a string.
        mindmap (str | None): Mindmap data as a string.
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("""
            INSERT INTO document_analysis (filename, username, faq, topics, mindmap)
            VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(filename, username) DO UPDATE SET
                faq=COALESCE(excluded.faq, document_analysis.faq),
                topics=COALESCE(excluded.topics, document_analysis.topics),
                mindmap=COALESCE(excluded.mindmap, document_analysis.mindmap),
                generated_at=CURRENT_TIMESTAMP
        """, (filename, username, faq, topics, mindmap))
        conn.commit()
        logger.info(f"Saved/updated analysis for document '{filename}' and user '{username}'.")
        return True
    except sqlite3.Error as e:
        logger.error(f"Database error saving analysis for document '{filename}', user '{username}': {e}", exc_info=True)
        if conn:
            conn.rollback()
        return False
    finally:
