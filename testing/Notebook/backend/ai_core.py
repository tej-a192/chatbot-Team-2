...
def extract_text_from_pdf(file) -> str | None:
    """Extracts text from a single PDF file using PyMuPDF (fitz).

    Args:
        pdf_path (str): The full path to the PDF file.
        pdf_path changed to file: The file object to read from.
    ...
    try:
        # Reset file pointer to start (important for re-reads)
        file.seek(0)
...
KG_OUTPUTS_FOLDER = os.path.join(BACKEND_DIR, 'KG_OUTPUTS') # Use the specified path

def get_kg_context_for_query(query: str, filename: str, username: str) -> str | None:
    """
    Retrieves relevant knowledge graph context for a given query, filename, and user.
    Loads KG JSON (expected to be a dict with 'nodes' and 'edges') and searches both.

    Args:
        query (str): The user's query string.
        filename (str): The name of the document file (e.g., 'document.pdf').
        username (str): The username associated with the document.

    Returns:
        str | None: A formatted string containing relevant KG data, or None if not found or error.
    """
    # --- Basic Input Validation ---
    if not query or not query.strip():
        logger.warning("[KG] Query is empty for context retrieval.")
        return None
    if not filename or not filename.strip():
        logger.warning(f"[KG] Filename is empty for KG context retrieval (user: {username}).")
        return None
    if not username or not username.strip():
        logger.warning(f"[KG] Username is empty for KG context retrieval (filename: {filename}).")
        return None

    sanitized_filename = os.path.basename(filename) # Ensure filename doesn't contain paths

    # Construct the path to the specific user's KG file (using filename.pdf.json format)
    # Ensure the .pdf extension is KEPT before adding .json
    # Example: 'UNIT-3_Notes.pdf' becomes 'UNIT-3_Notes.pdf.json'
    kg_file_path = os.path.join(KG_OUTPUTS_FOLDER, username, f"{sanitized_filename}.json")


    logger.info(f"[KG] Attempting to retrieve KG context for query '{query[:50]}...' (file: {sanitized_filename}, user: {username})")
    logger.debug(f"[KG] Looking for KG file at: {kg_file_path}")

    # --- Load and Parse KG Data ---
    kg_data = None
    try:
        if not os.path.exists(kg_file_path):
            logger.info(f"[KG] KG file not found at expected path: {kg_file_path}. Skipping KG context.")
            return None # File doesn't exist, no KG context available

        with open(kg_file_path, 'r', encoding='utf-8') as f:
            kg_data = json.load(f)
            # return kg_data
        logger.info(f"[KG] KG data loaded from {kg_file_path}: {kg_data}")

        # Validate the top-level structure
        if not isinstance(kg_data, dict) or "nodes" not in kg_data or "edges" not in kg_data:
             logger.error(f"[KG] KG data loaded from {kg_file_path} is not a dictionary with 'nodes' and 'edges' keys as expected. Type: {type(kg_data)}. Skipping KG context.")
             # Return an error message to the LLM to inform the user
             return f"Knowledge Graph Error: KG data for this document is in an unexpected format."

        nodes = kg_data.get("nodes", [])
        edges = kg_data.get("edges", [])

        if not isinstance(nodes, list) or not isinstance(edges, list):
             logger.error(f"[KG] KG data 'nodes' or 'edges' fields are not lists in {kg_file_path}. Skipping KG context.")
             return f"Knowledge Graph Error: KG data 'nodes' or 'edges' are not lists."


        if not nodes and not edges:
            logger.warning(f"[KG] KG data loaded from {kg_file_path} is empty (no nodes or edges).")
            return None # KG was empty

        logger.debug(f"[KG] Successfully loaded KG data from {kg_file_path}. Contains {len(nodes)} nodes and {len(edges)} edges.")

    except FileNotFoundError:
        # This is redundant with the initial check, but kept for safety
        logger.error(f"[KG] CRITICAL: FileNotFoundError during KG context retrieval after check: {kg_file_path}", exc_info=True)
        return None
    except json.JSONDecodeError as e:
        logger.error(f"[KG] Error decoding KG JSON file at {kg_file_path}: {e}", exc_info=True)
        # Return a specific error message to the LLM so it can inform the user
        return f"Knowledge Graph Error: Failed to read data for this document. Details: Invalid JSON format."
    except Exception as e:
        logger.error(f"[KG] Unexpected error during KG data loading/parsing for '{sanitized_filename}' (user: {username}): {e}", exc_info=True)
        # Return a specific error message to the LLM
        return f"Knowledge Graph Error: An unexpected issue occurred while loading KG data. Details: {type(e).__name__}"


    # --- Simple Search Logic within KG Data (Nodes and Edges) ---
    relevant_items = [] # Collect text representations of relevant data
    query_terms = [term.strip() for term in query.lower().split() if term.strip()] # Basic term splitting

    if not query_terms:
         logger.debug("[KG] Query terms are empty after splitting and stripping. Cannot perform KG search.")
         return None # Cannot search with empty terms

    logger.debug(f"[KG] Searching {len(nodes)} nodes and {len(edges)} edges for query terms: {query_terms}")

    # Search Nodes
    node_map = {n.get("id"): n for n in nodes if n.get("id")} # Map node id to node object for edge formatting
    for node in nodes:
         node_id = node.get("id")
         ...
         # Create a searchable string representation of the node
         searchable_text = f"{node_id} {label} {description} {' '.join(str(v) for v in properties.values())}".lower()

         if any(term in searchable_text for term in query_terms):
             # Format the relevant node info concisely
             props_str = ", ".join([f"{k}: {v}" for k, v in properties.items()])
             relevant_items.append(f"Node: '{label}' (ID: {node_id}) - Desc: '{description}' - Props: {props_str}")


    # Search Edges
    for edge in edges:
         if not isinstance(edge, dict):
             logger.warning(f"[KG] Found unexpected item in edges list (not a dict): {edge}. Skipping.")
             continue # Skip items that aren't dictionaries
         ...
         # Get labels from node_map for better context in searchable text
         from_label = node_map.get(from_node_id, {}).get("label", str(from_node_id)) # Use ID if label not found
         to_label = node_map.get(to_node_id, {}).get("label", str(to_node_id)) # Use ID if label not found

         # Create a searchable string representation of the edge
         searchable_text = f"{from_label} {relationship} {to_label} {' '.join(str(v) for v in properties.values())}".lower()

         if any(term in searchable_text for term in query_terms):
              # Format the relevant edge info concisely
              props_str = ", ".join([f"{k}: {v}" for k, v in properties.items()])
              relevant_items.append(f"Relation: '{from_label}' --('{relationship}')--> '{to_label}' - Props: {props_str}")


    if not relevant_items:
        logger.info(f"[KG] No relevant KG data found for query '{query[:50]}...' in '{sanitized_filename}'.")
        return None # Return None if no relevant info found

    # --- Format Relevant Info for LLM Context ---
    # Limit the number of relevant items to keep context size reasonable
    MAX_KG_ITEMS = 100 # Define a limit for relevant items (nodes + edges) to include
    formatted_context = "Relevant Knowledge Graph Data:\n"

    # Add formatted relevant items, limiting the total count
    for i, item_str in enumerate(relevant_items[:MAX_KG_ITEMS]):
         formatted_context += f"- {item_str}\n"

    logger.info(f"[KG] Found {len(relevant_items)} relevant KG items (nodes/edges). Including {min(len(relevant_items), MAX_KG_ITEMS)} in context.")
    return formatted_context.strip() # Return the formatted string
...
# --- MODIFIED: Added logging ---
def generate_document_analysis(filename: str, analysis_type: str, username: str, text: str) -> tuple[str | None, str | None]:
    """
    ...
    Args:
        ...
        analysis_type (str): The type of analysis requested ('faq', 'topics', or 'mindmap').
        username (str): The user who owns the document/requested the analysis.
        text (str): The extracted text content of the document. # <-- ADDED THIS ARGUMENT

    Returns:
        tuple[str | None, str | None]: (analysis_content, thinking_content)
                                    Returns (error_message, thinking_content) on failure.
                                    Returns (None, None) if document text cannot be found/loaded (though with 'text' arg, this path is less likely).
    """
    ...
    # --- Step 1: Use provided Document Text ---
    # We no longer need to retrieve from cache or disk here, as text is passed directly.
    # Keep caching logic in the caller (e.g., /upload) if needed elsewhere.
    doc_text = text # Use the text passed as an argument

    if not doc_text:
        # This check is less likely to be hit if text is passed from /upload,
        # but keep it for robustness if the caller somehow passes empty text.
        logger.error(f"Analysis failed: provided doc_text is empty for '{filename}'.")
        return f"Error: Provided text content for '{filename}' is empty.", None
