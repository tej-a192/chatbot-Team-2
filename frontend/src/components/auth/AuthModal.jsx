...
// <<< MODIFICATION: Added 'initialViewIsLogin' prop
function AuthModal({ isOpen, onClose, initialViewIsLogin = true }) {
    const { login, signup } = useAuth();
    const { setIsAdminSessionActive, switchLLM: setGlobalLLM, selectedLLM } = useAppState();
    
    // <<< MODIFICATION: Initialize state based on the new prop
    ...
    useEffect(() => {
        if (isOpen) {
            setError('');
            setStep(1);
            setIsLoginView(initialViewIsLogin); // <<< MODIFICATION: Reset view on open
            setFormData({
                email: '', password: '', localSelectedLLM: selectedLLM || 'gemini', apiKey: '',
                ollamaUrl: '', requestKeyFromAdmin: false, name: '', college: '',
                universityNumber: '', degreeType: 'Bachelors', branch: 'Computer Science',
                year: '1st Year', learningStyle: 'Visual', currentGoals: ''
            });
        }
    }, [isOpen, selectedLLM, initialViewIsLogin]); // <<< MODIFICATION: Added prop to dependency array

    // ... (rest of the file remains unchanged, paste the original content below)
    const handleChange = (e) => {
    const { name, value, type, checked } = e.target;
    setFormData(prev => {
        const newState = { ...prev, [name]: type === 'checkbox' ? checked : value };
...
// --- Helper for consistent input styling ---
    ...
    const handleSubmit = async (e) => {
        ...
        if (isLoginView) {
            ...
            try {
                const signupData = {
                    email: formData.email, password: formData.password, preferredLlmProvider: formData.localSelectedLLM,
                    requestAdminKey: formData.requestKeyFromAdmin, apiKey: formData.apiKey, ollamaUrl: formData.ollamaUrl,
                    name: formData.name, college: formData.college, universityNumber: formData.universityNumber,
                    degreeType: formData.degreeType, branch: formData.branch, year: formData.year,
                    learningStyle: formData.learningStyle.split(' ')[0], currentGoals: formData.currentGoals
                };
