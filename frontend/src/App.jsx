...
function MainAppLayout({ 
    ...
    <>
        ...
        <TopNav 
            ...
            onNewChat={() => handleNewChat(messages)}
...
function App() {
    ...
    // State to explicitly manage the onboarding process.
    ...
    useEffect(() => {
        const handleAuthAndSession = async () => {
            ...
            // If we are waiting for onboarding, we pause the entire application logic.
            // The app will wait until the onboarding flow calls its `onComplete` handler.
            ...
            if (regularUserToken && regularUser) {
                ...
                // For existing users who log in, we check their status from the context.
                if (regularUser.hasCompletedOnboarding === false) {
                    console.log("[App.jsx] User has not completed onboarding. Awaiting flow...");
                    setIsAwaitingOnboarding(true);
                    return; // Stop further processing until onboarding is done.
                }
                ...
                if (shouldCreateSession && !isCreatingSession) {
                    ...
                    await handleNewChat(null, true, true);
        ...
        isAwaitingOnboarding // Dependency ensures this effect re-runs after onboarding is complete.
    ]);
    ...
    const handleAuthSuccess = (authData) => {
        ...
        if (authData && !authData.isAdminLogin && authData.token) {
            ...
            if (authData.email && authData._id) {
                ...
                // Create the complete user object for the context from the API response.
                ...
                // Now, check if onboarding is needed based on the fresh data from the API.
                // This is especially for new signups.
                if (authData.isNewUser || authData.hasCompletedOnboarding === false) {
                    setIsAwaitingOnboarding(true);
                }
            }
        }
    ...
    return (
        <div className="flex flex-col h-screen overflow-hidden font-sans">
            <AnimatePresence>
                ...
                {/* The `onComplete` callback will set `isAwaitingOnboarding` to false, 
                    unlocking the app and allowing the main useEffect to proceed. */}
                {isAwaitingOnboarding && 
                    <OnboardingFlow onComplete={() => setIsAwaitingOnboarding(false)} />
                }
            </AnimatePresence>
            <Routes>
                {/* Only render the main app routes if we are NOT awaiting onboarding */}
                {!isAwaitingOnboarding && (regularUserToken && regularUser ? LoggedInRoutes : LoggedOutRoutes)}
            </Routes>
        </div>
    );
}
