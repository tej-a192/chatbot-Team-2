...
import Button from './components/core/Button.jsx';
import AcademicIntegrityPage from './components/tools/AcademicIntegrityPage.jsx';
import LandingPage from './components/landing/LandingPage.jsx'; // <<< NEW IMPORT
...
function App() {
    ...
    // <<< NEW: State to control if the modal is for login or signup ---
    ...
    useEffect(() => {
        const handleAuthAndSession = async () => {
            ...
            if (regularUserToken && regularUser) {
                ...
                if (shouldCreateSession && !isCreatingSession) {
                    setIsCreatingSession(true);
                    await handleNewChat(null, true, true); // Create a new session silently
                    setIsCreatingSession(false);
                }
            }
            // <<< REMOVED: No longer automatically show AuthModal on load
        };
        handleAuthAndSession();
    }, [
        regularUserAuthLoading, regularUserToken, regularUser, isAdminSessionActive, 
    ...
    const handleAuthSuccess = (authData) => {
        ...
        if (authData && !authData.isAdminLogin && authData.token) {
            ...
            navigate('/', { replace: true }); // Navigate to main app after login/signup
        }
    };

    // <<< NEW: Function to open the AuthModal, passed to LandingPage
    ...
    const LoggedOutRoutes = (
        <>
            {/* All logged-out users see the landing page */}
            ...
            <AnimatePresence>
                {showAuthModal && (
                    <AuthModal 
                        ...
                        // The AuthModal now needs to know if it should start in login or signup view.
                        // However, the existing AuthModal has its own internal state `isLoginView`.
                        // We'll pass a function to the LandingPage that sets the state here, and
                        // the modal can then receive an initial state. For simplicity, we'll
                        // modify the modal to accept an initial view prop later if needed, but the
                        // current implementation with `onLoginClick` should handle it.
                    />
                )}
            </AnimatePresence>
            <Routes>
                {regularUserToken && regularUser ? LoggedInRoutes : LoggedOutRoutes}
            </Routes>
        </div>
    );
}
